...
export function createFlightsRoutesLayer(map: Map, config: FlightsRoutesLayerConfig): void {
  ...
  map.addLayer({
    ...
    paint: {
      ...
      'line-width': ['interpolate', ['linear'], ['zoom'], 1, 1, 5, 2, 10, 3, 15, 4],
      'line-opacity': ['interpolate', ['linear'], ['zoom'], 1, 0.4, 5, 0.6, 10, 0.8],
    },
  });
  ...
  map.addLayer({
    ...
    paint: {
      'line-color': '#ffffff',
      'line-width': ['interpolate', ['linear'], ['zoom'], 1, 3, 5, 5, 10, 7, 15, 9],
      'line-opacity': 0.9,
    },
  });
}
